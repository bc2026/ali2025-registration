...
const safePushToDataLayer = (event) => {
  if (window.dataLayer) {
...
const StepTwo = ({ nextStep, handleFormData, prevStep, values }) => {
  ...
  useEffect(() => {
    ...
    safePushToDataLayer({ event: 'form_step_view', step: 'StepTwo' });

    return () => {
      const duration = Date.now() - start;
      safePushToDataLayer({ event: 'time_on_step', step: 'StepTwo', duration_ms: duration });
    };
  }, []);
  ...
  const submitFormData = async (e) => {
    ...
    if (
      ...
      safePushToDataLayer({ event: 'button_click', button: 'Submit', step: 'StepTwo' });
        ...
        <Form onSubmit={submitFormData}>
          {["email", "phone_no", "address", "residence_zip", "dob"].map((field, idx) => (
            <Form.Group className="mb-3" key={idx}>
              ...
              <Form.Control
                ...
                onFocus={() => safePushToDataLayer({ event: 'field_focus', field })}
                onBlur={(e) => safePushToDataLayer({ event: 'field_blur', field, value: e.target.value })}
          ...
          <div style={{ display: "flex", justifyContent: "space-around" }}>
            <Button
              variant="primary"
              onClick={() => {
                safePushToDataLayer({ event: 'button_click', button: 'Previous', step: 'StepTwo' });
                prevStep();
              }}
            >
              Previous
